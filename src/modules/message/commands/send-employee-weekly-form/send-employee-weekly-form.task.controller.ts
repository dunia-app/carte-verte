...
const route = 'send-employee-weekly-form'
const prodOnly = false

@Controller(route)
@Injectable()
@SkipJWTAuth()
export class SendEmployeeWeeklyFormController {
  constructor(
  ...
  @Post()
  async SendEmployeeWeeklyForm(@Response() res: FastifyReply): Promise<void> {
    ...
    try {
      ...
      const input: CommandProps<SendEmployeeWeeklyFormCommand> = {
        correlationId: cryptoRandomString({ length: 8 }),
      }

      const command = new SendEmployeeWeeklyFormCommand(input)
