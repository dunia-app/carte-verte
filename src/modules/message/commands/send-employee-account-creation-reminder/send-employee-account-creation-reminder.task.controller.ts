...
const route = 'send-employee-account-creation-reminder';
...
@Injectable()
@SkipJWTAuth()
export class SendEmployeeAccountCreationReminderController {
    constructor(
    ...
    @Post()
    async SendEmployeeAccountCreationReminder(@Response() res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const input: CommandProps<SendEmployeeAccountCreationReminderCommand> = {
                correlationId: cryptoRandomString({ length: 8 }),
            };

            const command = new SendEmployeeAccountCreationReminderCommand(input);
