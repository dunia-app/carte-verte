...
import { buildUnfreezeEmployeeRequests } from './unfreeze-employee.request'

describe('UnfreezeEmployee (Resolver)', () => {
  ...
  let requests: ReturnType<typeof buildUnfreezeEmployeeRequests>
  beforeAll(async () => {
    ...
    requests = buildUnfreezeEmployeeRequests(app)
    mockBaas(services.baas)
  })

  it('Should unfreeze the user and store the event', async () => {
    ...
    await requests
      .unfreezeEmployee(authInfo.user.id.value)
      .set(superAdminAuth.headers)
      .expect(
        expectEqualResult(
          (
            data: ResolverResponseClass<boolean, EmployeeAlreadyUnfrozenError>,
          ) => {
    ...
    const [employeeSaved, cardSaved, eventSaved] = await Promise.all([
      ...
      (
        ...
        .createQueryBuilder()
        .where({ eventName: 'EmployeeUnfrozenDomainEvent' })
        .andWhere('variables @> :variables', {
          variables: { aggregateId: authInfo.user.id.value },
        })
        .getOne(),
    ])
    expect(employeeSaved.getPropsCopy().freezedAt).toBeFalsy()
    expect(cardSaved.lockStatus).toBe(LockStatus.UNLOCK)
    expect(eventSaved).toBeTruthy()
  })

  it('Should not unfreeze the user and not store the event', async () => {
    const superAdminAuth = await loginAsSuperAdmin(app)
        ...
        expectEqualResult(
          (
            ...
            expect(data.error!.code).toBe(
              new EmployeeAlreadyUnfrozenError().code,
            )
          },
        ),
      )
