...
import { DistributeRetroactiveCashbackCommand } from "../../commands/distribute-retroactive-cashback/distribute-retroactive-cashback.command";
...
const route = 'distribute-retroactive-cashback';
...
@Injectable()
@SkipJWTAuth()
export class DistributeRetroactiveCashbackController {
    constructor(
    ...
    @Post()
    async DistributeRetroactiveCashback(@Response() res: FastifyReply): Promise<void> {
        ...
        try {
            redlock = await this.redis.redlock.lock(
                ...
                getCacheTime(10, true),
              )

            const input: CommandProps<DistributeRetroactiveCashbackCommand> = {
                correlationId: cryptoRandomString({ length: 8 }),
            };
    
            const command = new DistributeRetroactiveCashbackCommand(input);
