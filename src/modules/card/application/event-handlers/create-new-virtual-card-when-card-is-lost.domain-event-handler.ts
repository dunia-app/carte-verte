...
import { LostCardBlockedDomainEvent } from '../../domain/events/lost-card-blocked.domain-event'

export class CreateNewVirtualCardWhenCardIsLostDomainEventHandler extends DomainEventHandler {
  constructor(
    ...
    super(LostCardBlockedDomainEvent)
  }

  // Handle a Domain Event by perform changes to other aggregates (inside the same Domain).
  async handle(events: LostCardBlockedDomainEvent[]): Promise<void> {
