...
import { DestroyedCardBlockedDomainEvent } from '../../domain/events/destroyed-card-blocked.domain-event'

export class CreateNewVirtualCardWhenCardIsDestroyedDomainEventHandler extends DomainEventHandler {
  constructor(
    ...
    super(DestroyedCardBlockedDomainEvent)
  }

  // Handle a Domain Event by perform changes to other aggregates (inside the same Domain).
  async handle(events: DestroyedCardBlockedDomainEvent[]): Promise<void> {
