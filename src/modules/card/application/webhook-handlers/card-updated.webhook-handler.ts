...
import { CardUpdatedWebhookPayload } from '../../../../libs/ddd/infrastructure/baas/treezor-webhook.entity'
...
@Injectable()
export class CardUpdatedWebhookHandler extends TreezorWebhookHandler {
  constructor(protected readonly commandBus: CommandBus) {
    super(TreezorWebhookType.CARD_UPDATE)
  }

  // Webhook used when card is being locked from dashboard
  async handle(
    cardId: string,
    payload: CardUpdatedWebhookPayload,
  ): Promise<boolean> {
