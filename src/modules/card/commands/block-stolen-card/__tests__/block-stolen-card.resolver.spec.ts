...
import { buildBlockStolenCardRequests } from './block-stolen-card.request'

describe('BlockStolenCard (Resolver)', () => {
  ...
  let requests: ReturnType<typeof buildBlockStolenCardRequests>
  beforeAll(async () => {
    ...
    requests = buildBlockStolenCardRequests(app)
    mockBaas(services.baas)
  })

  it('Should block the card as stolen', async () => {
      ...
      .expect(
        expectEqualResult(
          (
            ...
            expect(data.result).toBe(LockStatus.STOLEN)
    ...
    const [cardSaved, eventSaved] = await Promise.all([
      ...
      (
        ...
        .createQueryBuilder()
        .where({ eventName: 'StolenCardBlockedDomainEvent' })
  ...
  it('Should not block the card as stolen as it is already blocked', async () => {
    const { authInfo, wallet, card } = await onboardAsEmployee(
      app,
      undefined,
      undefined,
      { lockStatus: LockStatus.DESTROYED, blockedAt: new DateVO(new Date()) },
    )
