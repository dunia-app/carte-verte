...
export async function blockStolenCard(
  command: BlockStolenCardCommand,
  ...
  try {
    ...
    const res = card.blockStolenCard()
    if (res.isErr) {
      return Result.err(res.error)
    }

    await baas.blockStolenCard(card.externalId, card.cardDigitalizationIds)
