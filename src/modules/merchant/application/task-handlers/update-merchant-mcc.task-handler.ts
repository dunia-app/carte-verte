...
import { UpdateMerchantMccCommand } from '../../commands/update-merchant-mcc/update-merchant-mcc.command';
const cryptoRandomString = require('crypto-random-string');
;
import Redlock = require('redlock');

const taskName = 'update-merchant-mcc'

@Injectable()
export class UpdateMerchantMccTaskHandler {
  constructor(
    private readonly commandBus: CommandBus,
    private readonly redis: RedisService,
  ) {}

  @ConditionalCron(CronExpression.EVERY_DAY_AT_3AM, {
    name: taskName,
  })
  async handleMerchantMccToBeUpdated() {
    let redlock: Redlock.Lock | undefined ;
    try {
      redlock = await this.redis.redlock.lock(
        `lock:[${this.constructor.name}]:${taskName}`,
        getCacheTime(55, true),
      )

      const now = new Date().toISOString()
      logger.info(`[${this.constructor.name}]: Start task ${taskName} at time ${now}`)

      const input: CommandProps<UpdateMerchantMccCommand> = {
        correlationId: cryptoRandomString({ length: 8 }),
      }
      const command = new UpdateMerchantMccCommand(input)
